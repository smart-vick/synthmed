...
// Rate limiter for public endpoints (100 req/15 min per IP)
export const publicLimiter = rateLimit({
  ...
  skip: (req) => req.auth?.tier === 'enterprise', // Enterprise unlimited
  standardHeaders: true,
  legacyHeaders: false,
});

// Rate limiter for auth endpoints (5 attempts/15 min per IP)
...
// Rate limiter for lead capture (5 per hour per IP)
...
export function createApiLimiter() {
  ...
  return Object.assign((req, res, next) => {
    ...
    const tierLimits = {
      free: 100,       // 100 per hour
      starter: 5000,   // 5,000 per hour
      pro: 50000,      // 50,000 per hour
